...
export function LineItemsTable({ lineItems }: LineItemsTableProps) {
  ...
  if (lineItems.length === 0) {
    return (
      <div className="overflow-hidden rounded-[24px] border border-[#d9c3a1] bg-[linear-gradient(180deg,rgba(255,255,255,0.88),rgba(252,247,240,0.95))] shadow-[0_16px_45px_rgba(188,151,98,0.08)] print:rounded-none print:shadow-none">
          ...
          {lineItems.map((item) => (
            <tr key={item.id} className="break-inside-avoid">
              ...
              <td className={cellClass}>
                ...
                <div className="mt-1.5 break-words text-xs text-[#8c7558] print:text-[9px]">{item.meta}</div>
              </td>
