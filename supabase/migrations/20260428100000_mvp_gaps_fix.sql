-- MVP Gaps Fix Migration
-- 1) Add invoice client snapshot columns
-- 2) Add invoice_items project snapshot column
-- 3) Make client email nullable
-- 4) Update create_invoice_with_items RPC to populate snapshots
...
-- ============================================================
-- 2. Add snapshot column to invoice_items
-- ============================================================
...
begin
  ...
  loop
    ...
    if coalesce(v_item ->> 'timeEntryId', '') <> '' then
      ...
      -- Fetch time entry ownership + project name for snapshot
      select proj.name
      into v_project_name
