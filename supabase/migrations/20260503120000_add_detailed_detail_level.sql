-- Add 'detailed' as a valid invoice detail level
--
-- Root cause: The TypeScript client sends detailLevel='detailed' (the only
-- value in the InvoiceDetailLevel type), but the DB check constraints and
-- the RPC function only accepted ('simple', 'standard', 'audit').
-- This caused every invoice creation via the UI to fail with HTTP 500.

-- 1. Loosen the invoices check constraint to include 'detailed'
alter table public.invoices
  drop constraint invoices_detail_level_check;

alter table public.invoices
  add constraint invoices_detail_level_check
  check (detail_level in ('simple', 'standard', 'audit', 'detailed'));

-- 2. Loosen the profiles check constraint to include 'detailed'
alter table public.profiles
  drop constraint profiles_default_invoice_detail_level_check;

alter table public.profiles
  add constraint profiles_default_invoice_detail_level_check
  check (default_invoice_detail_level in ('simple', 'standard', 'audit', 'detailed'));

-- 3. Update the RPC function to accept 'detailed' as a valid detail level
...
declare
  ...
  v_effective_detail_level text := coalesce(nullif(btrim(p_detail_level), ''), 'detailed');
...
begin
  ...
  if v_effective_detail_level not in ('simple', 'standard', 'audit', 'detailed') then
