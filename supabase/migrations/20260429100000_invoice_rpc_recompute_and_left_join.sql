-- Fix Issues 5 & 6: RPC amount recomputation + LEFT JOIN for projects
--
-- Issue 5: The RPC currently trusts client-supplied line item `amount` directly.
--          Fix: recompute v_item_amount from billedMinutes * hourlyRate server-side.
-- Issue 6: INNER JOIN between time_entries and projects causes INVALID_TIME_ENTRY_REFERENCE
--          when a time entry references a deleted or null project.
...
begin
  ...
  loop
    ...
    -- Issue 5 fix: recompute amount from billedMinutes and hourlyRate server-side
    ...
    if coalesce(v_item ->> 'timeEntryId', '') <> '' then
      ...
      -- Issue 6 fix: LEFT JOIN so missing projects don't fail the entire query
      select proj.name
      into v_project_name
